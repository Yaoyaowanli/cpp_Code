...
void Test4(){
    ...
    int& b = a;
    int* pa = &a;
    //通过调试可以看到，这里的a和b的值都是10，&a和&b都取到了同一个地址，说明引用都是同一块空间只不过多了一个名字
    //注：通过观察汇编发现，其实引用在底层汇编阶段是用指针来实现的。
    //而pa是a的指针，pa是一个变量，里面存的是a的地址，可以通过解引用操作a变量，但是pa也有自己的地址且和a不同。
}

//内联函数
//像这种swap函数会经常使用，而使用函数时，又会开辟栈帧造成消耗，如果不想让其消耗资源
// 在C中，使用宏函数来解决。2、C++使用内联函数来解决。
//加上关键字inline，这个函数就会在调用处展开，但在debug下不会展开，只有在release下才会在调用处展开
...
void Test6(){
    //c++11 auto 关键字 ：自动推导变量
    int a = 10;
    auto b = a;
    cout << "type a : " << typeid(a).name() <<endl <<"type b :" << typeid(b).name() <<endl;
    int& c = b;
